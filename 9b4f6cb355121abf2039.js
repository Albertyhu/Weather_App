function initMap() {
  ...
  const CONFIGURATION = {
    ...
    "capabilities": {
      "addressAutocompleteControl": false,
    ...
    geocoder.geocode({
      ...
      if (status == google.maps.GeocoderStatus.OK) {
        if (results[0]) {
          ...
          if (city_name !== '') getWeather(city_name, postal_code);
  ...
  for (const component of componentForm) {
    document.getElementById(component).addEventListener('blur', geocodeAddress);
    document.getElementById(component).addEventListener('keyup', function (event) {
  ...
  function geocodeAddress() {
    if (validateForm()) {
      let address = ''; // Get address from form inputs
